...
* `SITE_DUPLICATE` — exact (within float tolerance used for round-trip identity, not a
  chemistry judgment) coincidence of two sites under PBC.
...
* Coordination, distortion, composition/oxidation-state, disorder diagnostics — Phase
  3/4, each needs its own cited method/table.
