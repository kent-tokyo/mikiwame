...
/// exactly as they did before this change — but callers get told which
/// method ran (see [`PeriodicDistanceMethod`]) so a finding built from a
/// fallback-computed distance can say so, rather than reporting the same
/// unqualified confidence either way. Adopting `chematic_crystal::Lattice`'s
/// stricter condition-number criterion as `LATTICE_SINGULAR`'s own threshold
/// (it would also resolve the `LATTICE_EXTREME_ASPECT_RATIO` backlog item —
/// see `tasks/todo.md`) is a separate, future decision, not made here.
pub(crate) fn minimum_image(lattice: &[[f64; 3]; 3], a: [f64; 3], b: [f64; 3]) -> PeriodicDistance {
    match chematic_crystal::Lattice::from_matrix(*lattice) {
        ...
        Err(err) => PeriodicDistance {
            distance_angstrom: naive_minimum_image_distance(lattice, a, b),
...
/// The per-axis-rounding approximation `minimum_image` used exclusively
/// before this module started delegating to `chematic_crystal::minimum_image`.
/// Kept only as a fallback for lattices `chematic_crystal::Lattice::from_matrix`
/// rejects as near-singular — see `minimum_image`'s doc comment.
...
mod tests {
    ...
    fn minimum_image_distance_wraps_across_the_cell_boundary() {
        let a = 5.6402;
        let lattice = [[a, 0.0, 0.0], [0.0, a, 0.0], [0.0, 0.0, a]];
        let d = minimum_image(&lattice, [0.05, 0.0, 0.0], [0.95, 0.0, 0.0]);
    ...
    /// but exercised through the public `minimum_image` dispatcher — proves
    /// the `chematic_crystal` swap actually fixed the gap, not just that
    /// the crate's own tests claim to. Also pins that an ordinary (if
    /// strongly skewed) triclinic-ish cell like this one takes the exact
    /// path, not the fallback — the fallback is for lattices
    ...
    fn minimum_image_distance_finds_the_true_minimum_on_a_skewed_cell() {
        ...
        let found = minimum_image(&lattice, p, q);
    ...
    /// `minimum_image`'s doc comment) still has the historical per-axis-
    /// rounding gap. With nearly-parallel lattice vectors a and b, a
    /// legitimate periodic image (p vs. q shifted by one whole b vector) is
    ...
    /// `minimum_image`) since this same lattice takes the *exact* path
    /// through `minimum_image` (see the test above) — the fallback is
    /// reachable only via a lattice `Lattice::from_matrix` itself rejects,
    ...
    /// through — `minimum_image` must fall back, not panic, and must say
    /// so via `PeriodicDistanceMethod::ApproximateFallback`.
    #[test]
    fn minimum_image_falls_back_on_a_near_singular_lattice() {
        ...
        let d = minimum_image(&lattice, [0.0, 0.0, 0.0], [0.5, 0.0, 0.0]);
        match d.method {
    ...
    fn minimum_image_falls_back_on_an_extremely_short_lattice_vector() {
        ...
        let d = minimum_image(&lattice, [0.0, 0.0, 0.0], [0.0, 0.5, 0.0]);
        assert!(matches!(
            d.method,
            PeriodicDistanceMethod::ApproximateFallback { .. }
        ));
